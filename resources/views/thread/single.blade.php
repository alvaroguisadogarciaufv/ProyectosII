...
@section('content')
    <div class="content-wrap well">
        ...
        @can('update',$thread)
            <div class="actions">
                ...
                <a href="{{route('thread.edit',$thread->id)}}" class="btn btn-info btn-xs">Editar</a>
    ...
    @foreach($thread->comments as $comment)
        ...
        {{--reply to comment--}}
        <button class="btn btn-xs btn-default" onclick="toggleReply('{{$comment->id}}')">Responder</button>
        <br>
        {{--//reply form--}}
        <div style="margin-left: 50px" class="reply-form-{{$comment->id}} hidden">
            ...
            <form action="{{route('replycomment.store',$comment->id)}}" method="post" role="form">
                {{csrf_field()}}
                <legend>Generar respuesta</legend>
